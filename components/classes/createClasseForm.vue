  ...
  try {
    ...
    if (error) {
      ...
      toast.error("Erreur lors de la création", {
        description: h(
          "pre",
          { class: "mt-2 w-[440px] rounded-md bg-slate-950 p-4" },
          h("code", { class: "text-red-500" }, JSON.stringify(error, null, 2))
        ),
      });
    } else if (data) {
      toast.success("classe créée avec succès", {
        description: `L'classe "${data?.code}" a été ajoutée.`,
      });
      openForm.value = false;
    }
  } catch (error: any) {
        ...
        >
            ...
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  ...
                  class="border rounded-md placeholder:pl-2.5"
                  v-bind="componentField"
                />
              </FormControl>
              <FormMessage />
          ...
          <FormField v-slot="{ field }" name="lookup_id">
            <FormItem class="flex flex-col">
              <FormLabel>Type d'classe</FormLabel>
                      ...
                      {{
                        field.value
                          ? userTypeOptions.find(
                              (option) => option.lookup_id === field.value
                            )?.description
