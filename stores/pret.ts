...
export const usePretStore = defineStore('pret', {
  ...
  actions: {
    ...
    async updatePret(pret: Pret) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { data, error } = await supabase.from('pret').update(pret).eq('id', pret.id).select().single();
        if (error) throw error;
        if (data) {
    ...
    async deletePret(id: number) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { error } = await supabase.from('pret').delete().eq('id', id);
        if (error) throw error;
    ...
    genererEcheancier(pret: Pret): EcheancierPret[] {
      const { montant, duree, taux_interet_annuel, date_debut, id: pret_id } = pret;

      if (!montant || !duree || !taux_interet_annuel || !date_debut || !pret_id) {
        this.error = "Les informations du prêt sont incomplètes pour générer l'échéancier.";
        console.error(this.error);
        return [];
      }

      const taux_interet_mensuel = taux_interet_annuel / 12 / 100;
      const mensualite = (montant * taux_interet_mensuel * Math.pow(1 + taux_interet_mensuel, duree)) / (Math.pow(1 + taux_interet_mensuel, duree) - 1);

      const echeancier: EcheancierPret[] = [];
      let capital_restant_du = montant;
      let dateEcheance = new Date(date_debut);
      ...
      for (let i = 0; i < duree; i++) {
        ...
        echeancier.push({
          pret_id,
          date_echeance: new Date(dateEcheance).toISOString().split('T')[0], // Format YYYY-MM-DD
          montant_echeance: parseFloat(mensualite.toFixed(2)),
          principal_rembourse: parseFloat(principal_rembourse.toFixed(2)),
          interet_paye: parseFloat(interet_paye.toFixed(2)),
          capital_restant_du: parseFloat(Math.abs(capital_restant_du).toFixed(2)), // Utiliser Math.abs pour éviter -0.00
        });
      }
    ...
    async saveEcheancierComplet(echeancierComplet: EcheancierPret[]) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { data, error } = await supabase.from('echeancierPret').insert(echeancierComplet).select();
        if (error) throw error;
        if (data) {
    ...
    async createEcheancier(echeancier: EcheancierPret) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { data, error } = await supabase.from('echeancierPret').insert([echeancier]).select().single();
        if (error) throw error;
        if (data) {
    ...
    async fetchEcheanciers(pretId: number) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { data, error } = await supabase.from('echeancierPret').select('*').eq('pret_id', pretId);
        if (error) throw error;
    ...
    async updateEcheancier(echeancier: EcheancierPret) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { data, error } = await supabase.from('echeancierPret').update(echeancier).eq('id', echeancier.id).select().single();
        if (error) throw error;
        if (data) {
          const index = this.echeanciers.findIndex(e => e.id === echeancier.id);
    ...
    async deleteEcheancier(id: number) {
      ...
      try {
        const supabase = useSupabaseClient();
        const { error } = await supabase.from('echeancierPret').delete().eq('id', id);
        if (error) throw error;
