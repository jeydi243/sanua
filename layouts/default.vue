<template>
    <div>
        <SidebarProvider>
            <AppSidebar />
            <SidebarInset>
                <header class="flex h-16 shrink-0 items-center border border-b-[0.5px] border-dashed gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12">
                    <div class="flex items-center gap-2 px-4">
                        ...
                        <Separator orientation="vertical" class="mr-2 data-[orientation=vertical]:h-4" />
                        <Switch class="self-end" :model-value="colorMode.value === 'dark'" @update:model-value="toggleTheme">
                            <template #thumb>
                                <Icon v-if="colorMode.value === 'dark'" name="lucide:moon" class="size-3" />
