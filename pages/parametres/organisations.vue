<template>
  <div class="w-full">
    ...
    <span class="text-md"
      >List de toutes les organisations dans l'application
    </span>
    <div class="flex gap-2 items-center py-4">
      ...
      <Button @click="fetchOrganisations">
        ...
        <Icon
          v-if="!orgIsFetching"
          name="cuida:loading-right-outline"
          style="color: white"
        />
      </Button>
...
const orgIsFetching = ref(false);
const errorMessage = ref<null | PostgrestError | string>(null);

const fetchOrganisations = async () => {
  orgIsFetching.value = true;
  errorMessage.value = null;

  try {
    // Vous pouvez passer des paramètres de pagination ici, ex: ?page=2&perPage=50

    const { data: _organisations, error } = await supabase
      .from("organisation")
      .select("*");
