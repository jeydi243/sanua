<template>
    <div class="w-full p-4 md:p-8">
        ...
        <div class="flex items-center gap-2 py-4">
            <Input class="max-w-sm" placeholder="Filtrer par nom..." :model-value="table.getColumn('nom')?.getFilterValue() as string ?? ''" @update:model-value="table.getColumn('nom')?.setFilterValue($event)" />
        ...
        <div class="flex items-center justify-end space-x-2 py-4">
            <div class="flex-1 text-sm text-muted-foreground">{{ table.getFilteredSelectedRowModel().rows.length }} sur {{ table.getFilteredRowModel().rows.length }} ligne(s) sélectionnée(s).</div>
            <div class="space-x-2">
...
import { useClientStore } from '@/stores/client' // Assurez-vous que le type est exporté du store
...
const columns: ColumnDef<Client>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => h('div', new Date(row.getValue('date_naissance')).toLocaleDateString('fr-FR')),
    },
    {
